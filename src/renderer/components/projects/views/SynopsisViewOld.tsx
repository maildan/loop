                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <div className={`w-3 h-3 rounded-full ${
                                                element.plotRelevance && element.plotRelevance >= 4 
                                                    ? 'bg-red-500' 
...
export function SynopsisView({ synopsisId: propSynopsisId, onBack }: SynopsisViewProps): React.ReactElement {
    ...
    // 🔥 plotPoints 변경 시 자동 저장 (useEffect로 분리)
    useEffect(() => {
        if (plotPoints.length > 0) {
            saveToStore(plotPoints);
        }
    }, [plotPoints, saveToStore]);

    // 🔥 단순한 setPlotPoints 래퍼 (렌더링 중 사이드 이펙트 제거)
